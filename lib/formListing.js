...
const { SHEET_NAMES, FORM_LISTING_ROOT_FOLDER_ID } = require("./config");

const QUEUE_RANGE = "A2:N";

function formCredsOrThrow() {
  if (!FORM_LISTING_ROOT_FOLDER_ID) {
    const err = new Error("GOOGLE_DRIVE_FORM_LISTING_ROOT_FOLDER_ID is not set.");
    err.statusCode = 503;
    throw err;
  }
}
...
async function submitOneForm(agentCode, pin, fields, files) {
  ...
  return writeMutex.run(async () => {
    ...
    const monthFolder = await drive.getOrCreateChildFolder(FORM_LISTING_ROOT_FOLDER_ID, monthName);
