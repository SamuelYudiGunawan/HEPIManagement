...
// "judul" is appended last (not inserted among the existing columns) so a
// production sheet with rows already in it doesn't get its later columns
// silently shifted out of alignment — ensureSheet only ever repairs the
// header row, never re-shuffles existing data rows to match a new order.
...
const REVISION_HEADERS = [
  ...
  "status", "feedback", "date_created", "date_updated", "narasi"
];
...
// One row per (agent, device) — an agent logged in on two phones/browsers
// gets two rows, both notified. "endpoint" (the push service URL the
// browser gave us) is the natural unique key per device/browser install.
const PUSH_SUBSCRIPTION_HEADERS = ["kode_agen", "endpoint", "p256dh", "auth", "user_agent", "date_created"];
const NOTIFICATION_HEADERS = ["notification_id", "kode_agen", "title", "body", "url", "date_created"];
