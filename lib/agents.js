...
// A few listing-housekeeping actions (reassigning the owning agent, taking
// a listing down) are deliberately adminkantor-only, not admin — mirrors
// the same asymmetry already used for revision-request ownership bypass.
async function requireAdminKantor(agentCode) {
  const agent = await requireAgent(agentCode);
  if (String(agent.status || "").toLowerCase().trim() !== "adminkantor") {
    const err = new Error("Hanya adminkantor yang bisa akses.");
    err.statusCode = 403;
    throw err;
  }
  return agent;
