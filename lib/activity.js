...
async function getActiveAgentsForClosing(agentCode, pin) {
  await agents.requireAdmin(agentCode, pin);
  return agents.listActiveAgents();
}

async function submitClosings(agentCode, pin, payload) {
  payload = payload || {};
...
async function submitDailyActivity(agentCode, pin, payload) {
  payload = payload || {};
  return writeMutex.run(async () => {
...
async function checkDailyActivityDate(agentCode, pin, tanggal) {
  const agent = await agents.requireAgent(agentCode, pin);
...
async function getMonthlyAgentScores(agentCode, pin) {
  await agents.requireAgent(agentCode, pin);
  const values = await getActivityValues();
...
async function getAgentMonthHistory(agentCode, pin) {
  const agent = await agents.requireAgent(agentCode, pin);
  const values = await getActivityValues();
  const { year, month } = currentYearMonth();
