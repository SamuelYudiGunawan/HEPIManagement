...
async function ensureAgentSheet() {
  ...
  if (!found) {
    ...
    await appendValues(SHEET_NAMES.AGENTS, [["ADM1", "Admin Utama", "", "admin", "", "ya", ""]]);
    return;
  }
  const values = await getValues(SHEET_NAMES.AGENTS, "A1:G1");
  if (!values.length) {
    await updateValues(SHEET_NAMES.AGENTS, "A1", [AGENT_HEADERS]);
    await appendValues(SHEET_NAMES.AGENTS, [["ADM1", "Admin Utama", "", "admin", "", "ya", ""]]);
  }
...
async function ensureFormListingQueueSheet() {
  return ensureSheet(SHEET_NAMES.FORM_LISTING_QUEUE, FORM_LISTING_HEADERS);
}

async function ensureRevisionSheet() {
  return ensureSheet(SHEET_NAMES.REVISIONS, REVISION_HEADERS);
}

async function ensurePushSubscriptionSheet() {
  return ensureSheet(SHEET_NAMES.PUSH_SUBSCRIPTIONS, PUSH_SUBSCRIPTION_HEADERS);
}

async function ensureNotificationSheet() {
  return ensureSheet(SHEET_NAMES.NOTIFICATIONS, NOTIFICATION_HEADERS);
}

// rowNumber is 1-indexed (A1-notation style, e.g. the first data row is 2).
async function deleteRow(sheetTitle, rowNumber) {
  const sheets = await sheetsClient();
  const id = requireSheetsId();
  const meta = await getSheetMeta();
  const found = meta.find((p) => p.title === sheetTitle);
  if (!found) throw new Error("Sheet tidak ditemukan: " + sheetTitle);
  await withRetry(() => sheets.spreadsheets.batchUpdate({
    spreadsheetId: id,
    requestBody: {
      requests: [{
...
module.exports = {
  ...
  ensureFormListingQueueSheet,
  ensureRevisionSheet,
  ensurePushSubscriptionSheet,
