...
function extractHarga_(text, luasTanah, luasBangunan) {
  ...
  if (hargaHeading) {
    ...
    const hubungiMatch = afterHeading.match(/^\s*hubungi\b/im);
    const block = (hubungiMatch ? afterHeading.slice(0, hubungiMatch.index) : afterHeading)
      .split("\n").slice(0, 8).join("\n");
...
function insertBeforeHubungiOrEnd_(text, newLine) {
  text = String(text || "");
  const hubungiMatch = text.match(/^\s*hubungi\b/im);
  if (hubungiMatch) {
    return (text.slice(0, hubungiMatch.index) + newLine + "\n\n" + text.slice(hubungiMatch.index))
      .replace(/\n{3,}/g, "\n\n");
  }
  return (text.replace(/\n*$/, "") + "\n\n" + newLine + "\n");
}
...
function replaceHargaBlock_(text, newHargaLineText) {
  ...
  const hubungiMatch = afterHeading.match(/^\s*hubungi\b/im);
  const blockEndIdx = startIdx + (hubungiMatch ? hubungiMatch.index : afterHeading.length);
