...
// Raw bytes for a direct download (form-listing PDFs, property photos) —
// proxied through our own server rather than Drive's public share links, so
// it works regardless of link-sharing settings and needs no Google sign-in
// from the person clicking Download.
async function getFileBytes(fileId) {
  const meta = await getFile(fileId, "id,name,mimeType");
  const drive = await driveClient();
  const res = await withRetry(() => drive.files.get(
    { fileId, alt: "media", supportsAllDrives: true },
    { responseType: "arraybuffer" }
  ));
  return { buffer: Buffer.from(res.data), name: meta.name, mimeType: meta.mimeType };
}
...
// permanently deleting it — used when replacing a set of uploaded photos so
// a mistaken edit isn't unrecoverable.
