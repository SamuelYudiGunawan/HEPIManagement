...
function formatBulanTahunForFrontend(value) {
  ...
  if (typeof value === "number" && isFinite(value) && value > 1e12) {
...
function dateCreatedMs(value) {
  ...
  // Sheets can return a numeric serial as text depending on the API/value
  // render mode. Treat it the same as a numeric serial.
  if (/^\d+(?:\.\d+)?$/.test(raw)) {
    const numeric = Number(raw);
    if (numeric > 1e12) return numeric;
    if (numeric > 20000) return (numeric - 25569) * 86400 * 1000;
  }
  const localDate = raw.match(/^(\d{1,2})[\/-](\d{1,2})[\/-](\d{4})(?:\s+(\d{1,2}):(\d{2})(?::(\d{2}))?)?$/);
  if (localDate) {
    ...
    const seconds = Number(localDate[6] || 0);
