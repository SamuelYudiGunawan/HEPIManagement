...
async function runImport(forceFull, logger) {
  ...
  return importMutex.run(async () => {
    ...
    return writeMutex.run(async () => {
      ...
      // A full scan is also used for parser migrations. Clear the previous
      // watermark before scanning so existing files are re-parsed instead of
      // being skipped as "unchanged". If the scan hits the time limit, the
      // blank watermark stays in state and the resumed queue keeps doing the
      // same full re-parse until it drains.
      if (forceFull) state.lastImportTime = "";
      if (forceFull && !state.queueMode) state.queueMode = "manual";
...
async function handleCron(logger) {
  ...
  // The old importer could leave a large queue behind after its daily scan or
  // five-minute changed-file debounce. Those queues are intentionally not
  // resumed now that imports are manual between the weekly scans.
  if (Array.isArray(state.queue) && state.queue.length) {
    if (state.queueMode !== "weekly" && state.queueMode !== "manual") {
      state.queue = [];
      state.pendingImportAt = 0;
      saveState(state);
      return { ok: true, skipped: true, reason: "legacy import queue cleared" };
    }
    return runImport(false, log);
  }

  // One full scan per week, Sunday at 02:00 Asia/Jakarta. The cPanel cron
  // may call this endpoint more than once during the hour; the date guard
  // makes sure only the first call starts the scan.
  if (p.weekday === "Sun" && p.hour === 2) {
    const today = dates.ymd(now);
    if (state.lastFullScanDate !== today) {
      state.lastFullScanDate = today;
      state.queue = [ROOT_FOLDER_ID];
      state.queueMode = "weekly";
      saveState(state);
      return runImport(true, log);
    }
  }

  return { ok: true, skipped: true, reason: "weekly scan scheduled Sunday 02:00" };
}
...
async function auditImport() {
  ...
  const auditMaxMs = 25000;
  const values = await listings.getPropertyRows();
  const propertyIds = new Set(values.slice(1).map((row) => String((row || [])[0] || "").trim()).filter(Boolean));
  const discovered = [];
  const seenFolders = new Set();
  const queue = [ROOT_FOLDER_ID];
  let timedOut = false;
  while (queue.length) {
    if (Date.now() - startedAt > auditMaxMs) {
      timedOut = true;
      break;
    }
    const folderId = queue.shift();
    ...
    const meta = await drive.getFile(folderId, "id,name,webViewLink");
    const assets = await scanFolderAssets(folderId, meta);
    assets.txtFiles.forEach((file) => discovered.push({ id: file.id, name: file.name, folderId, folderLink: assets.folderUrl }));
    assets.subFolderIds.forEach((id) => queue.push(id));
  }
  const discoveredIds = new Set(discovered.map((file) => file.id));
  const nameCounts = {};
  discovered.forEach((file) => { nameCounts[file.name] = (nameCounts[file.name] || 0) + 1; });
  return {
    generatedAt: new Date().toISOString(),
    auditComplete: !timedOut && queue.length === 0,
    foldersScanned: seenFolders.size,
    sheetRows: Math.max(0, values.length - 1),
    driveNarratives: discovered.length,
    missingInSheet: discovered.filter((file) => !propertyIds.has(file.id)),
    // A timed-out audit only has a partial Drive index, so it cannot prove
    // that a Sheet row has no matching Drive file yet.
    missingInDrive: timedOut ? [] : Array.from(propertyIds).filter((id) => !discoveredIds.has(id)),
    duplicateNames: discovered.filter((file) => nameCounts[file.name] > 1),
    zeroOrMissingPrice: values.slice(1).map((row, i) => ({ rowNumber: i + 2, fileId: row[0] || "", fileName: row[1] || "", harga: Number(row[5]) || 0 })).filter((row) => row.harga <= 0)
  };
}

module.exports = { loadState, saveState, runImport, handleCron, getImportStatus, auditImport };
