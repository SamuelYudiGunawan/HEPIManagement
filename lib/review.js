...
async function listToday(agentCode) {
  const [allListings, revisions] = await Promise.all([
    ...
    revision.listForAdmin(agentCode)
  ]);
  ...
  (revisions.rows || []).forEach((row) => {
    if (dates.toDateKey(row.dateCreated) !== today) return;
    items.push({ kind: "revision", label: "Revisi", id: row.revisionId, title: row.namaListing || (listingById[row.fileId] && listingById[row.fileId].area) || row.fileId, agent: row.namaAgen || row.kodeAgen, status: row.status, dateCreated: row.dateCreated, url: "/revisi-review", revision: row, listing: listingById[row.fileId] || null });
  });
