  ...
  return writeMutex.run(async () => {
    ...
    if (files.foto && files.foto.length) {
      ...
      try {
        ...
        // best-effort naming only — the content replace below still
        // succeeds either way, just falls back to the generic name
      }

      await drive.replaceFileContent(thumbnailId, thumbName, mime, files.foto);
