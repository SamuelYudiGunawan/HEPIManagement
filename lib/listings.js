...
// Generic row lookup by fileId, returning enough (rowNumber + header) for a
// caller to target any single column via sheets.colLetter — shared by the
// Revision Request apply step and reassignAgent below, so both write
// targeted single cells instead of a full-row rewrite via upsertFast.
async function findPropertyRow(fileId) {
  fileId = String(fileId || "").trim();
  ...
  const fileIdCol = header.indexOf("file_id");
  for (let i = 1; i < values.length; i++) {
    if (String((values[i] || [])[fileIdCol]) === fileId) {
      const obj = {};
      header.forEach((h, idx) => { obj[h] = (values[i] || [])[idx]; });
      return { rowNumber: i + 1, header, obj };
    }
  }
  return null;
}

// Bulk version of findPropertyRow for callers that need to look up many
// listings at once (e.g. enriching a whole revision queue) — one sheet read
// instead of one per row.
async function getPropertyIndexByFileId() {
  const values = await getPropertyRows();
  const header = values[0] || [];
  const fileIdCol = header.indexOf("file_id");
  const map = {};
...
async function updatePropertyField(fileId, fieldName, value) {
  const found = await findPropertyRow(fileId);
  if (!found) throw new Error("Listing tidak ditemukan.");
  const colIdx = found.header.indexOf(fieldName);
  if (colIdx === -1) throw new Error("Kolom tidak ditemukan: " + fieldName);
  await sheets.updateValues(SHEET_NAMES.PROPERTIES, sheets.colLetter(colIdx) + found.rowNumber, [[value]]);
  return found;
}

async function reparseListing(agentCode, fileId) {
  return writeMutex.run(async () => {
    await agents.requireListingEditor(agentCode);
    const target = await findPropertyRow(fileId);
    if (!target) throw new Error("Listing tidak ditemukan.");
    const file = await drive.getFile(fileId, "id,name,mimeType,createdTime");
    const text = await drive.getFileText(fileId, file.mimeType);
    const agentDatabase = await agents.getAgentDataFromSheet(true);
    const data = parser.extractData(text, file.name, target.obj.bulantahun, agentDatabase);
    const values = await getPropertyRows();
    ...
    await upsertFast({
      ...
      fileName: file.name,
      data,
      thumbnail: target.obj.thumbnail || "",
      folderLink: target.obj.folder_link || "#",
      dateCreated: target.obj.date_created || file.createdTime || ""
    }, values, state);
    invalidateListings();
    return { ok: true, fileId, harga: data.harga, luasTanah: data.luasTanah, luasBangunan: data.luasBangunan };
  });
}

async function reassignAgent(callerAgentCode, targetFileId, newKodeAgen) {
  await agents.requireAdminKantor(callerAgentCode);
  const newKode = String(newKodeAgen || "").toUpperCase().trim();
  if (!newKode) throw new Error("Pilih agen baru.");

  const agentMap = await agents.getAgentDataFromSheet();
  const newAgent = agentMap[newKode];
  if (!newAgent) throw new Error("Agen tidak ditemukan: " + newKode);

  return writeMutex.run(async () => {
    await updatePropertyField(targetFileId, "agen", newAgent.nama || newKode);
    await updatePropertyField(targetFileId, "kontak", newAgent.hp || "");
    invalidateListings();
    return { ok: true, agen: newAgent.nama || newKode, kontak: newAgent.hp || "" };
  });
}

// "Takedown" — permanently removes a listing's row from "Data Properti".
// Does NOT touch the underlying Drive folder/files, only the sheet entry,
// so it's the site that stops showing it rather than an irreversible data
// loss on Drive.
async function deleteListing(callerAgentCode, targetFileId) {
  await agents.requireAdminKantor(callerAgentCode);
  return writeMutex.run(async () => {
    const target = await findPropertyRow(targetFileId);
    if (!target) throw new Error("Listing tidak ditemukan.");
    await sheets.deleteRow(SHEET_NAMES.PROPERTIES, target.rowNumber);
    invalidateListings();
    return { ok: true };
  });
}

// Listing folders/files follow "<prefix> <area words...> <harga> <kodeAgen>[ (n)]"
// (see submitOneListing) — prefix is always the first token and kodeAgen is
// always the last, so the area (everything between prefix and the last two
// tokens) and the harga (second-to-last token) can each be swapped
// independently without disturbing the other or needing to know the other's
// current value.
function renameAreaInFolderName_(oldName, newArea) {
  let base = String(oldName || "");
  let dedupSuffix = "";
  const dedupMatch = base.match(/^(.*)(\s\(\d+\))$/);
  if (dedupMatch) {
    base = dedupMatch[1];
    dedupSuffix = dedupMatch[2];
  }
  const tokens = base.split(" ").filter(Boolean);
  if (tokens.length < 4) return null; // prefix + >=1 area word + harga + kodeAgen
  const newAreaTokens = parser.sanitizeDriveName_(newArea).split(" ").filter(Boolean);
  if (!newAreaTokens.length) return null;
  const prefix = tokens[0];
  const tailTokens = tokens.slice(tokens.length - 2);
  return [prefix].concat(newAreaTokens, tailTokens).join(" ") + dedupSuffix;
}

// Formats an already-parsed rupiah amount into the same compact token
// submitOneListing's hargaForTitle_ produces at creation time (e.g.
// 1500000000 -> "1,5M", 150000000 -> "150JT"). Deliberately NOT built from
// the revision's raw free-text price (e.g. "1,5 M nego") the way
// hargaForTitle_ would be — hargaForTitle_ only rewrites known unit words
// and leaves everything else (like "nego") in place, which would leak an
// extra token into the folder name and corrupt any later rename that
// assumes the last token is kodeAgen and the second-to-last is harga.
// Working from the number extractHarga_ already parsed out sidesteps that
// entirely: the output here can never contain stray words.
function formatHargaFolderToken_(harga) {
  const n = Number(harga) || 0;
  if (n <= 0) return "";
  if (n >= 1000000000) {
    const rounded = Math.round((n / 1000000000) * 100) / 100;
    return String(rounded).replace(".", ",") + "M";
  }
  if (n >= 1000000) return String(Math.round(n / 1000000)) + "JT";
  return String(Math.round(n));
}

function renameHargaInFolderName_(oldName, hargaNumber) {
  let base = String(oldName || "");
  let dedupSuffix = "";
  const dedupMatch = base.match(/^(.*)(\s\(\d+\))$/);
  if (dedupMatch) {
  ...
  const tokens = base.split(" ").filter(Boolean);
  if (tokens.length < 4) return null; // prefix + >=1 area word + harga + kodeAgen
  const newHargaToken = formatHargaFolderToken_(hargaNumber);
  if (!newHargaToken) return null;
  const kodeAgenTok = tokens[tokens.length - 1];
  const areaTokens = tokens.slice(0, tokens.length - 2);
  return areaTokens.concat([newHargaToken, kodeAgenTok]).join(" ") + dedupSuffix;
}

// Shared by renameListingFolderForArea/renameListingFolderForHarga: given a
// listing's fileId and a function to compute its new folder name from the
// current one, renames the folder plus its .txt/.png. The .txt and
// thumbnail are looked up by their actual Drive file ID (row.fileId / the
// sheet's thumbnail column) rather than by guessing from the folder name,
// so they're renamed unconditionally to match — this self-heals listings
// whose folder was already renamed by hand before this existed (a real case
// seen in production: folder said "Sendangmulyo", sheet/.txt still said
// "Nusa Indah"). Only the folder-name parse itself can bail out (returns
// null) if the current folder name no longer fits the "<prefix> <area>
// <harga> <kodeAgen>" convention at all — in that case this is skipped and
// the caller's sheet/narrative update is unaffected.
async function renameListingFolder_(fileId, computeNewName) {
  try {
    const txtFile = await drive.getFile(fileId, "id,name,parents");
    const folderId = (txtFile.parents || [])[0];
    if (!folderId) return { renamed: false };

    // Some older listings (from before Jual/Sewa versions got their own
    // separate folders) still share one Drive folder between two listings'
    // worth of files — renaming it based on just one of those listings'
    // new area/harga would silently rename it out from under the other.
    // Bail out rather than guess whenever more than one narrative file
    // lives alongside this one in the same folder.
    const siblings = await drive.listChildren(folderId);
    const narrativeCount = siblings.filter((f) =>
      f.mimeType === "text/plain" || drive.isGoogleDocMime(f.mimeType)
    ).length;
    if (narrativeCount > 1) return { renamed: false, reason: "shared folder" };

    const folder = await drive.getFile(folderId, "id,name");
    const newFolderName = computeNewName(folder.name);
    if (!newFolderName) return { renamed: false };

    if (newFolderName !== folder.name) {
      await drive.renameFile(folderId, newFolderName);
    }

    const txtExt = (txtFile.name.match(/\.[a-z0-9]+$/i) || [".txt"])[0];
    const newTxtName = newFolderName + txtExt;
    if (txtFile.name !== newTxtName) {
      await drive.renameFile(fileId, newTxtName);
      await updatePropertyField(fileId, "file_name", newTxtName);
    }

    const target = await findPropertyRow(fileId);
    const thumbId = target && target.obj && target.obj.thumbnail;
    if (thumbId) {
      const thumbFile = await drive.getFile(thumbId, "id,name");
      const thumbExt = (thumbFile.name.match(/\.[a-z0-9]+$/i) || [".png"])[0];
      const newThumbName = newFolderName + thumbExt;
      if (thumbFile.name !== newThumbName) {
        await drive.renameFile(thumbId, newThumbName);
      }
    }

    return { renamed: true, folderName: newFolderName };
  } catch (e) {
    return { renamed: false, error: e.message || String(e) };
  }
}

async function renameListingFolderForArea(fileId, newArea) {
  return renameListingFolder_(fileId, (oldName) => renameAreaInFolderName_(oldName, newArea));
}

async function renameListingFolderForHarga(fileId, hargaNumber) {
  return renameListingFolder_(fileId, (oldName) => renameHargaInFolderName_(oldName, hargaNumber));
}

async function getNarrativeText(fileId) {
  try {
    const file = await drive.getFile(fileId, "id,name,mimeType");
    const isGoogleDoc = drive.isGoogleDocMime(file.mimeType);
    // Trust the actual mimeType Drive reports rather than the filename — a
    // narrative file that got manually renamed without its .txt extension
    // (a real case seen in production) is still genuinely text/plain and
    // perfectly readable; requiring the extension too was an unnecessary
    // second check that only made renaming mistakes elsewhere worse.
    if (!isGoogleDoc && file.mimeType !== "text/plain") {
      return { success: false, error: "Bukan file teks" };
    }
    const content = await drive.getFileText(fileId, file.mimeType);
...
async function submitOneListing(agentCode, payload, files) {
  ...
  return writeMutex.run(async () => {
    ...
    await drive.shareAnyoneReader(thumbFile.id);
...
module.exports = {
  ...
  findPropertyRow,
  reparseListing,
  getPropertyIndexByFileId,
  updatePropertyField,
  reassignAgent,
  deleteListing,
  renameListingFolderForArea,
  renameListingFolderForHarga
};
