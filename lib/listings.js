...
async function markAsSold(fileId, agentCode, pin) {
  ...
  return writeMutex.run(async () => {
    const loginResult = await agents.verifyAgentLogin(agentCode, pin);
    if (!loginResult.valid) {
      const err = new Error(loginResult.pesan || "🤣 wkwkwk... NGAWUR! SALAH!");
      err.statusCode = 401;
      throw err;
    }
    const agentMap = await agents.getAgentDataFromSheet();
    const currentAgent = agentMap[agentCode];
...
async function getActiveAgentsForListing(agentCode, pin) {
...
async function submitOneListing(agentCode, pin, payload, files) {
  payload = payload || {};
  files = files || {};
