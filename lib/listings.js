...
// (see submitOneListing) — prefix is always the first token and kodeAgen/harga
// are always the last two, so everything between them is the area, however
// many words it has. Swaps just that middle span for the new area.
...
// After a revision changes a listing's area, keeps the Drive folder name
// and its .txt/.png in sync. The .txt and thumbnail are looked up by their
// actual Drive file ID (row.fileId / the sheet's thumbnail column) rather
// than by guessing from the folder name, so they're renamed unconditionally
// to match the new folder name — this self-heals listings whose folder was
// already renamed by hand before this existed (a real case seen in
// production: folder said "Sendangmulyo", sheet/.txt still said "Nusa Indah").
// Only the folder-name parse itself can bail out (returns null) if the
// current folder name no longer fits the "<prefix> <area> <harga> <kodeAgen>"
// convention at all — in that case this is skipped and the caller's
// sheet/narrative update is unaffected.
async function renameListingFolderForArea(fileId, newArea) {
  try {
    ...
    const newFolderName = renameAreaInFolderName_(folder.name, newArea);
...
async function getNarrativeText(fileId) {
  try {
    ...
    if (!isGoogleDoc && !String(file.name || "").toLowerCase().endsWith(".txt")) {
