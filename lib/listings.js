...
// Best-effort: after a revision changes a listing's area, keeps the Drive
// folder name (and its .txt/.png, which are named to match the folder) in
// sync so the listing doesn't end up filed under its old area forever. If
// the current name doesn't fit the expected convention (e.g. renamed by
// hand before), this is silently skipped — the sheet/narrative update
// already applied is unaffected either way.
async function renameListingFolderForArea(fileId, newArea) {
  try {
    ...
    if (!newFolderName || newFolderName === folder.name) return { renamed: false };

    const oldFolderName = folder.name;
    await drive.renameFile(folderId, newFolderName);

    if (txtFile.name === oldFolderName + ".txt") {
      await drive.renameFile(fileId, newFolderName + ".txt");
    }
    ...
    if (thumbId) {
      ...
      if (thumbFile.name === oldFolderName + ".png") {
        await drive.renameFile(thumbId, newFolderName + ".png");
      }
    }
